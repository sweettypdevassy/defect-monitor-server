#!/bin/bash
# Script to check defect database statistics
# Can be run directly on terminal

docker compose exec defect-monitor python3 -c "
import sqlite3
...
if total_defects == 0:
    print('⚠️  No defects in database yet')
    conn.close()
    exit(0)
...
print('\n' + '=' * 80)
print('📊 TOP 10 COMPONENTS BY DEFECT COUNT')
print('=' * 80)
...
if rows:
    for defect_id, summary, component, creation_date, has_desc in rows:
    ...
    print('\n  ✅ No defects with missing data found!')
